...
class Ucpaas
{
	...
	private function connection($url, $body, $type, $method)
	{
		...
		if (function_exists('curl_init')) {
			$header = array('Accept:' . $mine, 'Content-Type:' . $mine . ';charset=utf-8', 'Authorization:' . $this->getAuthorization());
			$ch = curl_init($url);
	...
	public function templateSMS($appId, $to, $templateId, $param = null, $type = 'json')
	{
		...
		if ($type == 'json') {
			$body_json = array('templateSMS' => array('appId' => $appId, 'templateId' => $templateId, 'to' => $to, 'param' => $param));
