...
//decode by QQ:465420700 http://www.0766city.com/
...
class dayu_yuyuepayModuleSite extends WeModuleSite
{
	...
	public function get_Authorization_Code($code)
	{
		...
		if (empty($token) || !is_array($token) || empty($token['access_token']) || empty($token['openid'])) {
			$oauth2_code = $this->createMobileUrl('list', array(), true);
			header("location:$oauth2_code");
	...
	{
		$menus = array(array('title' => '新建预约主题', 'url' => $this->createWebUrl('post'), 'icon' => 'fa fa-plus-square'), array('title' => '预约主题列表', 'url' => $this->createWebUrl('display'), 'icon' => 'fa fa-list'), array('title' => '预约记录管理', 'url' => $this->createWebUrl('manage'), 'icon' => 'fa fa-clock-o'), array('title' => '幻灯片管理', 'url' => $this->createWebUrl('slide'), 'icon' => 'fa fa-photo'), array('title' => '客服管理', 'url' => $this->createWebUrl('staff'), 'icon' => 'fa fa-wechat'), array('title' => '统计', 'url' => $this->createWebUrl('summary'), 'icon' => 'fa fa-signal'));
		return $menus;
	}
	public function getHomeTiles()
	{
		...
		if (!empty($list)) {
			foreach ($list as $row) {
				$urls[] = array('title' => $row['title'], 'url' => $_W['siteroot'] . "app/" . $this->createMobileUrl('dayu_yuyuepay', array('id' => $row['reid'])));
			}
		}
		return $urls;
	}
	...
	public function doWebCategory()
	{
		...
		if ($operation == 'display') {
			...
			foreach ($category as $key => $val) {
				...
				$category[$key]['link'] = murl('entry', array('do' => 'list', 'id' => $val['id'], 'm' => 'dayu_yuyuepay'), true, true);
			...
			if (checksubmit('submit')) {
				...
				$data = array('weid' => $weid, 'title' => $_GPC['title'], 'list' => $_GPC['list']);
				if (!empty($_GPC['thumb'])) {
					$data['icon'] = $_GPC['thumb'];
					load()->func('file');
					file_delete($_GPC['thumb-old']);
				}
				$color = array('nav_index' => $_GPC['nav_index'], 'nav_page' => $_GPC['nav_page'], 'nav_btn' => $_GPC['nav_btn']);
	...
	public function doWebQRcode()
	{
		...
		$value = $_W['siteroot'] . 'app/' . $this->createMobileUrl('list', array('cateid' => $id));
		$errorCorrectionLevel = "L";
		$matrixPointSize = "20";
		$imgname = $id . "s.png";
		$imgurl = $QRcode_path . $imgname;
		QRcode::png($value, $imgurl, $errorCorrectionLevel, $matrixPointSize);
		$category = pdo_get($this->tb_category, array('weid' => $weid, 'id' => $id), array());
	...
	public function doWebDetail()
	{
		...
		if ($_W['ispost']) {
			...
			$data = array('first' => array('value' => $par['mfirst'] . "\n", 'color' => "#743A3A"), 'keyword1' => array('value' => $info['member']), 'keyword2' => array('value' => $xiangmu), 'keyword3' => array('value' => $times), 'keyword4' => array('value' => $status['name']), 'remark' => array('value' => $kfinfo . "\n" . $activity['mfoot'], 'color' => "#008000"));
			if ($par['sms'] != '0' && !empty($activity['smsid']) && $info['status'] == '0') {
				load()->func('communication');
				$content = $activity['title'] . '，' . $par['xmname'] . ':' . $xm['title'] . '，状态：' . $status['name'];
				ihttp_post(murl('entry', array('do' => 'Notice', 'id' => $activity['smsid'], 'm' => 'dayu_sms'), true, true), array('mobile' => $info['mobile'], 'mname' => $info['member'], 'product' => $xiangmu, 'status' => $status['name']));
			}
			...
			if ($setting['store'] == 1 && pdo_tableexists('dayu_yuyuepay_plugin_store_store') && !empty($info['sid']) && $_GPC['status'] == '3') {
				...
				$paylog = pdo_get('dayu_yuyuepay_plugin_store_paylog', array('weid' => $_W['uniacid'], 'sid' => $info['sid'], 'bid' => $store['bid'], 'reid' => $info['reid'], 'yid' => $info['rerid']), array());
				if (empty($paylog)) {
					$paylogdata = array('weid' => $_W['uniacid'], 'sid' => $info['sid'], 'bid' => $store['bid'], 'reid' => $info['reid'], 'yid' => $info['rerid'], 'price' => $info['price'], 'createtime' => TIMESTAMP);
	...
	public function doWebAllManage()
	{
		global $_W, $_GPC;
		require 'fans.web.php';
		...
		$where .= "reid IN ('" . implode("','", is_array($yuyueid) ? $yuyueid : array($yuyueid)) . "')";
		$params = array();
		if (!empty($_GPC['time'])) {
		...
		$sql = 'SELECT * FROM ' . tablename($this->tb_info) . " WHERE $where ORDER BY rerid desc";
		...
		if (!empty($_GPC['export'])) {
			...
			foreach ($list as $key => $value) {
				$data[$key]['activity'] = $this->get_yuyuepay($value['reid']);
				$data[$key]['cate'] = $this->get_category($data[$key]['activity']['cid']);
			...
			foreach ($data as $value) {
				$html .= $value['cate']['title'] . "\t,";
				$html .= $value['activity']['title'] . "\t,";
				$html .= $value['member'] . "\t,";
				$html .= $value['mobile'] . "\t,";
				$html .= $value['xmid']['title'] . "\t,";
				$html .= $value['price'] . "\t,";
				$html .= $value['activity']['is_time'] == 2 ? $value['restime'] . "\t," : $value['yuyuetime'] . "\t,";
			...
			$name = $_GPC['status'] == '' ? '全部：' : $status['name'] . '：';
			header("Content-type:text/csv");
			header("Content-Disposition:attachment; filename=$name$stime-$etime.csv");
			echo $html;
			exit;
		}
		include $this->template('allmanage');
	}
	public function doWebManage()
	{
		...
		$pager = pagination($total, $pindex, $psize);
		$paytype = array('0' => array('css' => 'default', 'name' => '未支付'), '1' => array('css' => 'success', 'name' => '在线支付'), '2' => array('css' => 'info', 'name' => '余额支付'), '3' => array('css' => 'warning', 'name' => '其他付款方式'), '4' => array('css' => 'info', 'name' => '免费预约'));
	...
	public function doWebBatchrRcord()
	{
		...
		$reply = pdo_fetch("select reid from " . tablename($this->tb_yuyue) . " where reid = :reid", array(':reid' => $reid));
		if (empty($reply)) {
			$result['status'] = 0;
	...
	public function doWebSetStatus()
	{
		...
		$data = array('status' => $state, 'yuyuetime' => TIMESTAMP);
		foreach ($_GPC['idArr'] as $k => $rerid) {
			pdo_update($this->tb_info, $data, array('reid' => $reid, 'rerid' => $rerid));
		}
		$result['status'] = 1;
		$result['msg'] = '批量更新成功！';
		message($result, '', 'ajax');
		...
		if ($op == 'list') {
			...
			if (checksubmit('submit')) {
				if (!empty($_GPC['ids'])) {
					foreach ($_GPC['ids'] as $k => $v) {
						...
						pdo_update($this->tb_item, $data, array('reid' => $reid, 'id' => intval($v)));
					}
					message('编辑成功', $this->createWebUrl('item', array('op' => 'list', 'reid' => $reid)), 'success');
				}
			}
			...
			if (!$id) {
				if (checksubmit('submit')) {
					...
					message('阿^莫^之^家：添加服务项目成功', $this->createWebUrl('item', array('reid' => $reid, 'op' => 'list')), 'success');
				}
				$item = array("isshow" => 1);
			} else {
				$item = pdo_fetch('SELECT * FROM ' . tablename($this->tb_item) . ' WHERE weid = :weid AND reid = :reid AND id = :id', array(':weid' => $weid, ':reid' => $reid, ':id' => $id));
				if (checksubmit('submit')) {
					$data['title'] = $_GPC['title'];
					$data['thumb'] = $_GPC['thumb'];
					$data['type'] = intval($_GPC['type']);
					...
					$data['content'] = trim($_GPC['content']);
					$data['isshow'] = intval($_GPC['isshow']);
					pdo_update($this->tb_item, $data, array('reid' => $reid, 'id' => $id));
					message('编辑服务项目成功', $this->createWebUrl('item', array('reid' => $reid, 'op' => 'list')), 'success');
				}
			}
			include $this->template('item');
		} elseif ($op == 'edit') {
			...
			if (checksubmit('submit')) {
				...
				message('修改服务项目成功', $this->createWebUrl('item', array('reid' => $reid, 'op' => 'list')), 'success');
			}
			include $this->template('item');
		} elseif ($op == 'itemdel') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				pdo_delete($this->tb_item, array('id' => $id));
			}
	...
	public function doWebSlide()
	{
		...
		if ($operation == 'display') {
			...
			$id = intval($_GPC['id']);
			if (checksubmit('submit')) {
				$data = array('weid' => $_W['uniacid'], 'title' => $_GPC['title'], 'link' => $_GPC['link'], 'cid' => $_GPC['cate'], 'enabled' => intval($_GPC['enabled']), 'displayorder' => intval($_GPC['displayorder']));
				if (!empty($_GPC['thumb'])) {
					$data['thumb'] = $_GPC['thumb'];
					load()->func('file');
					file_delete($_GPC['thumb-old']);
	...
	public function doMobileList22()
	{
		...
		foreach ($projects as $index => $v) {
			$projects[$index]['link'] = $v['is_time'] == 2 ? $this->createMobileUrl('timelist', array('id' => $v['reid'])) : $this->createMobileUrl('dayu_yuyuepay', array('id' => $v['reid']));
		}
		$jquery = 2;
	...
	public function doMobileList()
	{
		...
		$yuyue = pdo_fetchAll("SELECT reid,weid,title,is_time,icon,thumb,subtitle,status,description,content,is_list,displayorder FROM" . tablename($this->tb_yuyue) . " WHERE status=1 AND weid='{$weid}' $is_list $where order by displayorder DESC,reid DESC");
		foreach ($yuyue as $index => $v) {
			$yuyue[$index]['link'] = $v['is_time'] == 2 ? $this->createMobileUrl('timelist', array('id' => $v['reid'])) : $this->createMobileUrl('dayu_yuyuepay', array('id' => $v['reid']));
		}
		include $this->template('index');
	...
	public function doMobileGetYuyue()
	{
		global $_GPC, $_W;
		$weid = $_W['uniacid'];
		$yuyue = pdo_get($this->tb_yuyue, array('weid' => $weid, 'reid' => $_GPC['id']), array());
		$par = iunserializer($yuyue['par']);
		$link = $yuyue['is_time'] == 2 ? $this->createMobileUrl('timelist', array('id' => $yuyue['reid'])) : $this->createMobileUrl('dayu_yuyuepay', array('id' => $yuyue['reid']));
		$mylink = $this->createMobileUrl('mydayu_yuyuepay', array('id' => $yuyue['reid']));
	...
	public function doMobileGetSlider()
	{
		...
		$slide = pdo_fetchall("SELECT * FROM " . tablename($this->tb_slide) . " WHERE weid = :weid and enabled=1 $where ORDER BY displayorder DESC", array(':weid' => $weid));
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_slide) . " WHERE weid = :weid and enabled=1 $where", array(':weid' => $weid));
		$data = array();
		foreach ($slide as $index => $val) {
			$slide[$index]['thumb'] = tomedia($val['thumb']);
			$slide[$index]['indicator'] = $index == '0' ? ' mui-active' : '';
			$data[] = array('id' => $val['id'], 'title' => $val['title'], 'thumb' => tomedia($val['thumb']), 'link' => $val['link'], 'indicator' => '<div class="mui-indicator' . $slide[$index]['indicator'] . '"></div>', 'slide' => '<div class="mui-slider-item"><a href="' . $slide[$index]['mode'] . '"><img src="' . $slide[$index]['thumb'] . '"><p class="mui-slider-title f-white">' . $val['title'] . '</p></a></div>');
		}
	...
	{
		global $_GPC, $_W;
		$field = array("key" => 11, "cyid" => 22, "startday" => 33, "endday" => 44, "page" => 55, "limit" => 66);
	...
	public function payResult($params)
	{
		...
		pdo_update($this->tb_info, $data, array('ordersn' => $params['tid']));
		if ($params['from'] == 'return') {
			$order = pdo_fetch("SELECT * FROM " . tablename($this->tb_info) . " WHERE ordersn = :ordersn", array(':ordersn' => $params['tid']));
			$activity = $this->get_yuyuepay($order['reid']);
			$par = iunserializer($activity['par']);
			if (!empty($par['credit'])) {
				load()->model('mc');
				$log = $activity['title'] . '-预约奖励' . $par['credit'] * $params['fee'] . '积分';
				mc_credit_update(mc_openid2uid($order['openid']), 'credit1', $par['credit'] * $params['fee'], array(0, $log));
				mc_group_update(mc_openid2uid($order['openid']));
			...
			if ($par['sms'] != '1' && !empty($activity['smsid'])) {
				load()->func('communication');
				ihttp_post(murl('entry', array('do' => 'Notice', 'id' => $activity['smsid'], 'm' => 'dayu_sms'), true, true), array('mobile' => $activity['mobile'], 'mname' => $order['member'], 'product' => $xiangmu, 'status' => $keyword2));
			}
			if (!empty($order['sid']) && !empty($activity['store']) && !empty($settings['store'])) {
				$store = $this->get_store($order['sid']);
				$kfid = $store['openid'];
				$staff = pdo_fetchall("SELECT `openid` FROM " . tablename('dayu_yuyuepay_plugin_store_store') . " WHERE weid=:weid and id=:id", array(':weid' => $_W['uniacid'], ':id' => $order['sid']));
				$address = '\\n' . $store['name'] . '：' . $store['loc_p'] . $store['loc_c'] . $store['loc_a'] . $store['address'];
			} else {
				$kfid = $activity['kfid'];
				$staff = pdo_fetchall("SELECT `openid` FROM " . tablename($this->tb_staff) . " WHERE weid=:weid and reid=:reid", array(':weid' => $_W['uniacid'], ':reid' => $order['reid']));
				$address = '';
			}
				...
				if ($settings['stime'] < $ptime && $settings['etime'] > $ptime) {
					if ($settings['newtemp'] == 1) {
						if (!empty($activity['k_templateid'])) {
							$template = array("touser" => $kfid, "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $_W['uniacid'], 'id' => $order['rerid'], 'reid' => $order['reid'])), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode($f_s . "\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', $order['createtime'])), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($keyword2), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $order['member'] . "\\n手机：" . $order['mobile'] . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time), 'color' => "#01579b")));
							$this->send_template_message(urldecode(json_encode($template)));
						}
					} else {
						if (!empty($activity['k_templateid']) && is_array($staff)) {
							foreach ($staff as $s) {
								$template = array("touser" => $s['openid'], "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $_W['uniacid'], 'id' => $order['rerid'], 'reid' => $order['reid'])), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode($f_s . "\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', $order['createtime'])), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($keyword2), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $order['member'] . "\\n手机：" . $order['mobile'] . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time), 'color' => "#01579b")));
								$this->send_template_message(urldecode(json_encode($template)));
							}
						}
					}
			...
			if (!empty($activity['m_templateid'])) {
				$data = array("touser" => $order['openid'], "template_id" => $activity['m_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('detail', array('reid' => $order['reid'], 'id' => $order['rerid'])), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode($f_c . "\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode($order['member']), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($xiangmu), 'color' => '#EF4F4F'), 'keyword3' => array('value' => urlencode($ytime), 'color' => '#EF4F4F'), 'keyword4' => array('value' => urlencode("订单号：" . $params['tid'] . $transaction . $address), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n" . $activity['mfoot']), 'color' => "#01579b")));
			...
			if (!empty($activity['mobile']) && $activity['smsid'] != 0) {
				...
				ihttp_post(murl('entry', array('do' => 'Notice', 'id' => $activity['smsid'], 'm' => 'dayu_sms'), true, true), array('mobile' => $activity['mobile'], 'mname' => $order['member'], 'mmobile' => $order['mobile'], 'content' => $content));
			}
			$uni_setting = uni_setting($_W['uniacid'], array('creditbehaviors'));
			$credit = $uni_setting['creditbehaviors']['currency'];
			$tishi = $getxm['price'] == '0.00' ? '提交预约成功！' : '支付成功！';
			if ($params['type'] == $credit) {
				$this->showMessage($tishi, $this->createMobileUrl('mydayu_yuyuepay', array('weid' => $_W['uniacid'], 'id' => $order['reid'])), 'success');
			} else {
				Message($tishi, $this->createMobileUrl('mydayu_yuyuepay', array('weid' => $_W['uniacid'], 'id' => $order['reid'])), 'success');
			}
		}
	}
	public function doMobilePay()
	{
		...
		$order = pdo_fetch("SELECT * FROM " . tablename($this->tb_info) . " WHERE rerid = :rerid", array(':rerid' => $orderid));
		$xm = pdo_fetch("SELECT * FROM " . tablename($this->tb_item) . " WHERE weid = :weid and reid = :reid and id = :xmid", array(':weid' => $_W['uniacid'], ':reid' => $order['reid'], ':xmid' => $order['xmid']));
		if ($order['paystatus'] != '1') {
			$this->showMessage('抱歉，您的预约已经付款或是被关闭，请查看预约列表。', $this->createMobileUrl('mydayu_yuyuepay', array('weid' => $order['weid'], 'id' => $order['reid'])), 'error');
		}
		if ($order['price'] == '0.00') {
		...
		$params['virtual'] = 1;
		$log = pdo_get('core_paylog', array('uniacid' => $_W['uniacid'], 'module' => 'dayu_yuyuepay', 'tid' => $params['tid']));
		if (empty($log)) {
	...
	public function doWebdayu_yuyuepayDelete()
	{
		...
		require 'fans.web.php';
		$id = intval($_GPC['id']);
		$info = pdo_get($this->tb_info, array('rerid' => $id), array());
		if (empty($info)) {
			message('访问非法.');
		}
		$activity = $this->get_yuyuepay($info['reid']);
		...
		if (!empty($id)) {
			pdo_delete($this->tb_info, array('rerid' => $id));
			pdo_delete($this->tb_data, array('rerid' => $id));
		}
		message('删除成功.', referer());
	}
	public function doWebPost()
	{
		...
		if ($op == 'post') {
			...
			if (checksubmit()) {
				$data = array('today' => $_GPC['today'], 'member' => intval($_GPC['member']), 'smsid' => intval($_GPC['yzm']), 'credit' => intval($_GPC['credit']), 'credit2' => intval($_GPC['credit2']), 'state1' => trim($_GPC['state1']), 'state2' => trim($_GPC['state2']), 'state3' => trim($_GPC['state3']), 'state4' => trim($_GPC['state4']), 'state5' => trim($_GPC['state5']), 'state8' => trim($_GPC['state8']), 'submit' => trim($_GPC['submits']), 'mname' => trim($_GPC['mname']), 'xmname' => trim($_GPC['xmname']), 'numname' => trim($_GPC['numname']), 'yuyuename' => trim($_GPC['yuyuename']), 'allnum' => intval($_GPC['allnum']), 'kfirst' => trim($_GPC['kfirst']), 'kfoot' => trim($_GPC['kfoot']), 'mfirst' => trim($_GPC['mfirst']), 'mfoot' => trim($_GPC['mfoot']), 'edit' => intval($_GPC['edit']), 'sms' => $_GPC['sms'], 'comment' => intval($_GPC['comment']));
				$switch = array();
				$record = array();
				...
				if (empty($reid)) {
					...
					if (pdo_update($this->tb_yuyue, $record, array('reid' => $reid)) === false) {
						message('保存预约失败, 请稍后重试.');
					}
				}
				pdo_delete($this->tb_group, array('weid' => $_W['uniacid'], 'reid' => $reid));
				if ($_GPC['group'] && $reid) {
					foreach ($_GPC['group'] as $gid) {
						$gid = intval($gid);
						$insert = array('weid' => $_W['uniacid'], 'reid' => $reid, 'groupid' => $gid);
			...
			if (!$activity) {
				$par = array("state1" => "待受理", "state2" => "受理中", "state3" => "已完成", "state4" => "拒绝受理", "state5" => "已取消", "state8" => "退回修改", "mname" => "我的预约", "submit" => "立 即 提 交", "xmname" => "服务项目", "numname" => "数量", "yuyuename" => "预约时间", "kfirst" => "有新的客户预约，请及时确认", "kfoot" => "点击确认预约，或修改预约时间", "mfirst" => "预约结果通知", "mfoot" => "如有疑问，请致电联系我们");
				$activity = array("information" => "您的预约申请我们已经收到, 请等待客服确认.", "phone" => "手机", "pretotal" => "0", "day" => 12, "status" => 1, "pay" => 2, "follow" => 1, "is_time" => 2, "is_list" => 1, "is_addr" => 0, "daynum" => 0, "timelist" => 1, "skins" => "weui", "isthumb" => 1, "endtime" => date('Y-m-d H:i:s', strtotime('+365 day')));
			}
		} elseif ($op == 'verify') {
			if ($_W['isajax']) {
	...
	public function doWebStaff()
	{
		...
		if ($op == 'list') {
			...
			if (checksubmit('submit')) {
				if (!empty($_GPC['ids'])) {
					foreach ($_GPC['ids'] as $k => $v) {
						$data = array('nickname' => trim($_GPC['nickname'][$k]), 'openid' => trim($_GPC['openid'][$k]), 'weid' => $weid);
						pdo_update($this->tb_staff, $data, array('reid' => $reid, 'id' => intval($v)));
					}
			...
			if (checksubmit('submit')) {
				...
				message('&#x963F;.&#x83AB;.&#x4E4B;.&#x5BB6;.&#x63D0;.&#x793A;&#xFF1A;&#x6DFB;&#x52A0;&#x5BA2;&#x670D;&#x6210;&#x529F;', $this->createWebUrl('staff', array('reid' => $reid, 'op' => 'list')), 'success');
			}
			include $this->template('staff');
		} elseif ($op == 'staffdel') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				pdo_delete($this->tb_staff, array('weid' => $weid, 'id' => $id));
			}
			message('删除成功.', referer());
		}
	}
	public function doWebchangecheckedAjax2()
	{
		...
		$kfid = $_GPC['kfid'];
		if (!empty($kfid)) {
			if (false !== pdo_update($this->tb_yuyue, array('kfid' => $kfid), array('reid' => intval($id), 'weid' => $_W['uniacid']))) {
				exit('1');
			} else {
				exit('0');
			}
	...
	public function doWebchangecheckedAjax()
	{
		...
		$change = $_GPC['change'];
		if ($field == 'kfid') {
			if (false !== pdo_update($this->tb_yuyue, array('kfid' => $change), array('reid' => intval($id), 'weid' => $_W['uniacid']))) {
				exit('1');
			} else {
				exit('0');
			}
		...
		if ($field == 'guanli') {
			if (false !== pdo_update($this->tb_yuyue, array('guanli' => $change), array('reid' => intval($id), 'weid' => $_W['uniacid']))) {
				exit('1');
			} else {
				exit('0');
			}
	...
	public function doMobileTimelist()
	{
		...
		$project = pdo_get($this->tb_yuyue, array('reid' => $id, 'weid' => $weid, 'status' => 1), array('reid', 'weid', 'is_time', 'timelist', 'srvtime', 'day', 'remove', 'follow', 'out1', 'out2', 'out3', 'out4', 'out5', 'out6', 'out7', 'restrict', 'endtime', 'store'));
		if ($project['is_time'] != 2) {
			header('Location: ' . $this->createMobileUrl('dayu_yuyuepay', array('id' => $project['reid'])), true, 301);
		}
		if ($project['follow'] == 1) {
			$this->getFollow();
		}
		...
		$link = $this->createMobileUrl('todayTime', array('id' => $id));
		...
		if ($activity['is_addr'] == 1 && !$address) {
			$this->showMessage('请先完善您的资料', $this->createMobileUrl('address', array('from' => 'dayu_yuyuepay', 'returnurl' => $returnUrl)), 'info');
		}
		...
		if ($project['restrict'] == 1) {
			$havs = pdo_fetchall("SELECT restime,num,SUM(num) AS rescount FROM " . tablename($this->tb_info) . " WHERE ((status='0' and paystatus='2') or (status='0' and paystatus='1' and createtime >= '{$paytime}') or status='1' or status='3') AND reid=:id GROUP BY restime", array(':id' => $id), 'restime');
		} else {
			$havs = pdo_fetchall("SELECT restime,count(reid) as rescount from " . tablename($this->tb_info) . " WHERE ((status='0' and paystatus='2') or (status='0' and paystatus='1' and createtime >= '{$paytime}') or status='1' or status='3') AND reid=:id GROUP BY restime", array(':id' => $id), 'restime');
		}
		$dates = array();
		$now = new DateTime();
		$today = $par['today'];
		...
		$isma = pdo_get($this->tb_yuyue, array('weid' => $weid, 'kfid' => $openid), array());
	...
	public function doMobiletodayTime()
	{
		...
		$out6 = array_map('change_to_quotes', explode(',', $project['out6']));
		$out7 = array_map('change_to_quotes', explode(',', $project['out7']));
		$link = $_W['siteroot'] . 'app/' . $this->createMobileUrl('Timelist', array('id' => $id));
		$weekarray = array("周日", "周一", "周二", "周三", "周四", "周五", "周六");
		if (empty($datetime)) {
			$timelist = json_decode($project['srvtime'], TRUE);
	...
	public function doMobiledayu_yuyuepay()
	{
		...
		if (!empty($_GPC['repeat'])) {
			if (!empty($repeat)) {
				if ($repeat == $_GPC['repeat']) {
					$this->showMessage($activity['information'], $this->createMobileUrl('mydayu_yuyuepay', array('weid' => $weid, 'id' => $reid)));
				} else {
		...
		$ii = $_GPC['ii'];
		$srvtime = base64_decode($datetime);
		$xms = pdo_fetchall("SELECT * FROM " . tablename($this->tb_item) . " WHERE weid = :weid and reid = :reid and isshow=1 ORDER BY displayorder DESC,id DESC", array(':weid' => $_W['uniacid'], ':reid' => $reid));
		$xmstotal = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_item) . " WHERE weid = :weid and reid = :reid and isshow=1", array(':weid' => $_W['uniacid'], ':reid' => $reid));
		$title = $activity['title'];
		...
		if ($activity['pretotal'] != 0) {
			$pretotal = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE reid = :reid AND openid = :openid", array(':reid' => $reid, ':openid' => $_W['openid']));
		...
		if ($activity['iscard'] == 1 || $activity['iscard'] == 2) {
			...
			$card_url = $activity['iscard'] == 1 ? $this->createMobileUrl('mycard', array('a' => 'card', 'c' => 'mc', 'i' => $weid, 'returnurl' => $returnUrl), false) : murl('entry', array('do' => 'index', 'm' => 'dayu_card', 'returnurl' => $returnUrl), true, true);
			if ($ishy == false) {
		...
		$address = pdo_fetch("SELECT * FROM " . tablename('mc_member_address') . " WHERE isdefault = 1 and uid = :uid limit 1", array(':uid' => $_W['fans']['uid']));
		if ($activity['is_addr'] == 1 && !$address) {
			$this->showMessage('请先完善资料', $this->createMobileUrl('address', array('from' => 'dayu_yuyuepay', 'returnurl' => $returnUrl)), 'info');
		}
		$mgroup = pdo_fetch("SELECT * FROM " . tablename('mc_members') . " WHERE uniacid = '{$weid}' AND uid = '{$uid}'");
		...
		foreach ($ds as &$r) {
			...
			if ($r['type'] == 'tingli' && pdo_tableexists('dayu_tingli')) {
				$r['tingli'] = pdo_fetch("SELECT * FROM " . tablename('dayu_tingli') . " WHERE weid = :weid AND openid = :openid ORDER BY id DESC", array(':weid' => $weid, ':openid' => $openid));
				$r['tingliurl'] = murl('entry', array('do' => 'index', 'm' => 'dayu_tingli', 'returnurl' => $returnUrl), true, true);
		...
		$xm = pdo_fetch("SELECT * FROM " . tablename($this->tb_item) . " WHERE id = :id", array(':id' => $_GPC['xmid']));
		if (pdo_tableexists('dayu_yuyuepay_plugin_store_store') && !empty($sid) && !empty($activity['store'])) {
			$store = pdo_get('dayu_yuyuepay_plugin_store_store', array('weid' => $weid, 'id' => $sid), array());
			$store['score_num'] = $store['score_num'] == 0 ? 5 : round($store['total_score'] / $store['score_num'], 0);
			if (pdo_tableexists('dayu_ishare_record')) {
				$share_record = pdo_get('dayu_ishare_record', array('weid' => $weid, 'openid' => $openid, 'gid' => $sid, 'addons' => 'dayu_yuyuepay_plugin_store'), array());
				$get_share = $share_record['share'];
			}
		}
		$paytime = strtotime("now") - $setting['paytime'] * 60;
		$timelist = json_decode($activity['srvtime'], TRUE);
		$havsnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE ((status=0 and paystatus=2) or (status=0 and paystatus=1 and createtime >= '{$paytime}') or status=1 or status=3) AND reid = :reid AND restime LIKE '%{$srvtime}%'", array(':reid' => $reid));
		$timenum = $timelist['times'][$ii]['number'];
		if ($_W['ispost']) {
			if ($activity['is_time'] == 2) {
				if ($_GPC['havsnum'] >= $_GPC['timenum']) {
					$this->showMessage('已约满，重新选择时间', $this->createMobileUrl('timelist', array('id' => $reid, 'datetime' => $datetime)), 'info');
				}
			}
			...
			if (empty($_GPC['restime']) && $activity['is_time'] == 2) {
				$this->showMessage('预约时间不能为空，请点击下面的链接返回选择预约时间', $this->createMobileUrl('timelist', array('id' => $reid)), 'error');
			}
			...
			if ($par['member'] == 0 && $activity['is_addr'] == 1) {
				...
				$infourl = $this->createMobileUrl('address', array('from' => 'dayu_yuyuepay', 'returnurl' => $returnUrl));
			} else {
				$member = $_GPC['member'];
				$mobile = $_GPC['mobile'];
				$address = "";
				$infourl = $this->createMobileUrl('info', array('from' => 'dayu_yuyuepay', 'returnurl' => $returnUrl));
			}
			if (!$mobile || !$member) {
			...
			foreach ($_GPC as $key => $value) {
				if (strexists($key, 'field_')) {
					...
					if ($refid && $field) {
						...
						if (in_array($activity['skins'], array('weui', 'weui_yuntai'))) {
							if (in_array($field['type'], array('number', 'text', 'calendar', 'email', 'textarea', 'radio', 'range', 'select', 'image', 'reside', 'checkbox', 'photograph', 'tingli'))) {
								$entry['data'] = strval($value);
							}
						} else {
							if (in_array($field['type'], array('number', 'text', 'calendar', 'email', 'textarea', 'radio', 'range', 'select', 'image', 'reside', 'tingli'))) {
								$entry['data'] = strval($value);
							}
			...
			if (empty($activity['starttime'])) {
				...
				pdo_update($this->tb_yuyue, $record, array('reid' => $reid));
			}
			if ($activity['is_time'] == 0) {
			...
			if (!empty($_GPC['sid'])) {
				$store = pdo_get('dayu_yuyuepay_plugin_store_store', array('weid' => $weid, 'id' => $_GPC['sid']), array());
				$kfid = $store['openid'];
				$staff = pdo_fetchall("SELECT `openid` FROM " . tablename('dayu_yuyuepay_plugin_store_store') . " WHERE weid=:weid and id=:id", array(':weid' => $weid, ':id' => $_GPC['sid']));
				$address = '\\n' . $store['name'] . '：' . $store['loc_p'] . $store['loc_c'] . $store['loc_a'] . $store['address'];
			} else {
				$kfid = $activity['kfid'];
				$staff = pdo_fetchall("SELECT `openid` FROM " . tablename($this->tb_staff) . " WHERE weid=:weid and reid=:reid", array(':weid' => $weid, ':reid' => $reid));
				$address = '';
			}
			if ($activity['is_time'] != 1) {
			...
			if ($setting['stime'] != 0 && $setting['etime'] != 0) {
				if ($setting['stime'] < $ptime && $setting['etime'] > $ptime) {
					if ($setting['newtemp'] == 1 && $setting['notice'] == 0) {
						if (!empty($activity['k_templateid'])) {
							$template = array("touser" => $kfid, "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $weid, 'id' => $row['rerid'], 'reid' => $row['reid'], 'sid' => $sid)), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("粉丝：" . $fans['nickname'] . "提交了新订单\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', TIMESTAMP)), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($paytype), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $member . "\\n手机：" . $mobile . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time . $bodnew), 'color' => "#01579b")));
							$this->send_template_message(urldecode(json_encode($template)));
						}
					} elseif ($setting['newtemp'] == 0 && $setting['notice'] == 0) {
						if (!empty($activity['k_templateid']) && is_array($staff)) {
							foreach ($staff as $s) {
								$template = array("touser" => $s['openid'], "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $weid, 'id' => $row['rerid'], 'reid' => $row['reid'], 'sid' => $sid)), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("粉丝：" . $fans['nickname'] . "提交了新订单\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', TIMESTAMP)), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($paytype), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $member . "\\n手机：" . $mobile . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time . $bodnew), 'color' => "#01579b")));
								$this->send_template_message(urldecode(json_encode($template)));
							}
						}
					}
				...
				if ($setting['newtemp'] == 1 && $setting['notice'] == 0) {
					if (!empty($activity['k_templateid'])) {
						$template = array("touser" => $kfid, "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $_W['uniacid'], 'id' => $row['rerid'], 'reid' => $row['reid'], 'sid' => $sid)), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("粉丝：" . $fans['nickname'] . "提交了新订单\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', TIMESTAMP)), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($paytype), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $member . "\\n手机：" . $mobile . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time . $bodnew), 'color' => "#01579b")));
						$this->send_template_message(urldecode(json_encode($template)));
					}
				} elseif ($setting['newtemp'] == 0 && $setting['notice'] == 0) {
					if (!empty($activity['k_templateid']) && is_array($staff)) {
						foreach ($staff as $s) {
							$template = array("touser" => $s['openid'], "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('weid' => $_W['uniacid'], 'id' => $row['rerid'], 'reid' => $row['reid'], 'sid' => $sid)), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("粉丝：" . $fans['nickname'] . "提交了新订单\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', TIMESTAMP)), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($paytype), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $member . "\\n手机：" . $mobile . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $address . $time . $bodnew), 'color' => "#01579b")));
							$this->send_template_message(urldecode(json_encode($template)));
						}
					}
				}
			...
			if ($_GPC['paytype'] == 1) {
				$url = $this->createMobileUrl('pay', array('orderid' => $orderid, 'weid' => $_GPC['weid'], 'id' => $_GPC['id']));
				Header("Location: $url");
			} else {
				if ($_GPC['paytype'] == 9 || $activity['pay'] == 1) {
					$this->showMessage($activity['information'], $this->createMobileUrl('mydayu_yuyuepay', array('weid' => $row['weid'], 'id' => $row['reid'])), 'success');
				}
			}
		}
		load()->func('tpl');
	...
	public function doMobilechecktime()
	{
		...
		$paytime = strtotime("now") - $setting['paytime'] * 60;
		$pretotal = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE ((status='0' and paystatus='2') or (status='0' and paystatus='1' and createtime >= '{$paytime}') or status='1' or status='3') AND reid = :reid AND xmid = :xmid AND restime like '%{$restime}%'", array(':reid' => $reid, ':xmid' => $_GPC['xmid'], ':restime' => $restime));
	...
	public function doMobilegetnum()
	{
		...
		$project = pdo_fetch($sql);
		$timelist = json_decode($project['srvtime'], TRUE);
		$pretotal = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE reid = :reid AND restime = :restime", array(':reid' => $reid, ':restime' => $restime));
	...
	public function doMobilegetprice()
	{
		...
		$setting = $this->module['config'];
		$paytime = strtotime("now") - $setting['paytime'] * 60;
		$item = pdo_fetch("SELECT * FROM " . tablename($this->tb_item) . " WHERE weid = :weid AND id = :id AND isshow=1", array(':weid' => $_W['uniacid'], ':id' => $_GPC['xmid']));
		...
		$info = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE ((status=0 and paystatus=2) or (status=0 and paystatus=1 and createtime >= '{$paytime}') or status=1 or status=3) AND reid = :reid AND xmid = :xmid AND restime like '%{$itime}%'", array(':reid' => $item['reid'], ':xmid' => $item['id']));
		if ($item['daynum'] == 0) {
			$result['status'] = 1;
			...
			if ($item['daynum'] != 0 && $info < $item['daynum']) {
				$result['status'] = 1;
				$result['type'] = $item['type'];
				$result['price'] = $item['price'];
				$result['prices'] = $item['prices'];
		...
		if ($op == 'num') {
			$num = pdo_fetch("SELECT SUM(num) AS rescount FROM " . tablename($this->tb_info) . " WHERE ((status=0 and paystatus=2) or (status=0 and paystatus=1 and createtime >= '{$paytime}') or status=1 or status=3) AND reid=:id AND restime like '%{$_GPC['time']}%'", array(':id' => $item['reid']));
	...
	public function doMobileMydayu_yuyuepay()
	{
		...
		$activity = $this->get_yuyuepay($reid);
		$par = iunserializer($activity['par']);
		$main_url = $par['member'] == 0 ? $this->createMobileUrl('list') : $this->createMobileUrl('dayu_yuyuepay', array('id' => $reid));
		$index_url = !empty($activity['store']) && pdo_tableexists('dayu_yuyuepay_plugin_store_store') ? murl('entry', array('do' => 'store', 'm' => 'dayu_yuyuepay_plugin_store'), true, true) : $main_url;
		$par = iunserializer($activity['par']);
		$period = $_GPC['period'];
		...
		$params = array(':reid' => $reid, ':openid' => $openid, ':starttime' => strtotime($starttime), ':endtime' => strtotime($endtime));
		$status = $_GPC['status'];
		if ($_GPC['status'] != '') {
			if ($status == 2) {
				$where .= " and ( status=2 or status=9 )";
		...
		$paytype = array('0' => array('css' => 'default', 'name' => '未支付'), '1' => array('css' => 'green', 'name' => '在线支付'), '2' => array('css' => 'blue', 'name' => '余额支付'), '3' => array('css' => 'black ', 'name' => '其他付款方式'), '4' => array('css' => 'orange', 'name' => '免费预约'), '9' => array('css' => 'primary', 'name' => '线下付款'));
	...
	public function doMobiledetail()
	{
		...
		$info_rd = pdo_fetchall('SELECT * FROM ' . tablename($this->tb_record) . ' WHERE rerid = :rerid ORDER BY id DESC', array(':rerid' => $row['rerid']));
		...
		$main_url = $par['is_list'] == 1 ? $this->createMobileUrl('list') : $this->createMobileUrl('dayu_yuyuepay', array('id' => $row['reid']));
		$index_url = !empty($activity['store']) && pdo_tableexists('dayu_yuyuepay_plugin_store_store') ? murl('entry', array('do' => 'store', 'm' => 'dayu_yuyuepay_plugin_store'), true, true) : $main_url;
	...
	public function doMobileEdit()
	{
		...
		foreach ($field as $f) {
			...
			$f['type'] == 'photograph' && empty($profile[$f['bind']]) && $this->showMessage('请完善' . $f['title'], murl('entry', array('do' => 'index', 'f' => $f['bind'], 'm' => 'dayu_photograph', 'returnurl' => $returnUrl), true, true), 'info');
			pdo_tableexists('dayu_photograph_fields') && ($f['photograph_url'] = murl('entry', array('do' => 'index', 'f' => $f['bind'], 'm' => 'dayu_photograph', 'returnurl' => $returnUrl), true, true));
			if ($f['type'] == 'tingli' && pdo_tableexists('dayu_tingli')) {
				$f['tingli'] = pdo_fetch("SELECT * FROM " . tablename('dayu_tingli') . " WHERE weid = :weid AND openid = :openid", array(':weid' => $weid, ':openid' => $openid));
				empty($f['tingli']) && $this->showMessage('请完成进行听力测试', murl('entry', array('do' => 'index', 'm' => 'dayu_tingli', 'returnurl' => $returnUrl), true, true), 'info');
		...
		if ($_W['ispost'] || checksubmit('submit')) {
			...
			if (!empty($datas)) {
				...
				if (is_array($staff)) {
					foreach ($staff as $s) {
						$template = array("touser" => $s['openid'], "template_id" => $activity['k_templateid'], "url" => murl('entry', array('do' => 'manage_detail', 'id' => $info['rerid'], 'reid' => $info['reid'], 'sid' => $info['sid'], 'm' => 'dayu_yuyuepay'), true, true), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("订单：" . $info['ordersn'] . "已修改\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', TIMESTAMP)), 'color' => '#000000'), 'keyword2' => array('value' => urlencode($status['name']), 'color' => '#000000'), 'keyword3' => array('value' => urlencode($ytime), 'color' => '#000000'), 'keyword4' => array('value' => urlencode($bodym . "\\n"), 'color' => "#FF0000"), 'remark' => array('value' => urlencode("\\n姓名：" . $info['member'] . "\\n手机：" . $info['mobile'] . "\\n" . $par['xmname'] . "：" . $xiangmu . $bodnew), 'color' => "#008000")));
						load()->func('communication');
						$this->send_template_message(urldecode(json_encode($template)));
					}
				}
			}
			$this->showMessage('修改成功，请等待受理', $this->createMobileUrl('mydayu_yuyuepay', array('id' => $reid)));
		}
		load()->func('tpl');
	...
	public function doMobilemembers()
	{
		...
		$dhcoupon_url = pdo_tableexists('we7_coupon') ? murl('entry', array('do' => 'activity', 'type' => 'coupon', 'm' => 'we7_coupon'), true, true) : url('activity/coupon') . 'wxref=mp.weixin.qq.com#wechat_redirect';
		$mycoupon_url = pdo_tableexists('we7_coupon') ? murl('entry', array('do' => 'activity', 'op' => 'mine', 'type' => 'coupon', 'm' => 'we7_coupon'), true, true) : url('activity/coupon/mine') . 'wxref=mp.weixin.qq.com#wechat_redirect';
	...
	public function doMobilemanager()
	{
		...
		$list = pdo_fetchall("SELECT s.reid, y.* FROM " . tablename($this->tb_staff) . " s left join " . tablename($this->tb_yuyue) . " y on y.reid=s.reid WHERE y.weid=:weid AND s.openid=:openid ORDER BY y.reid DESC", array(':weid' => $weid, ':openid' => $openid), 'reid');
		foreach ($list as $key => $val) {
	...
	public function doMobilemanage()
	{
		...
		if ($setting['store'] == 1 && empty($sid)) {
			$this->showMessage("店铺ID异常", murl('entry', array('do' => 'store', 'm' => 'dayu_yuyuepay_plugin_store'), true, true), 'error');
		} elseif ($setting['store'] == 1 && !empty($sid) && $openid != $store['openid']) {
			$this->showMessage("你不是店主", murl('entry', array('do' => 'store', 'm' => 'dayu_yuyuepay_plugin_store'), true, true), 'error');
		...
		if ($setting['store'] == 1) {
			$list = pdo_fetchall("SELECT * FROM " . tablename('dayu_yuyuepay_plugin_store_store') . " WHERE weid = :weid and checked = 1 and openid = :openid ORDER BY id DESC", array(':weid' => $weid, ':openid' => $openid), 'id');
			foreach ($list as $key => $val) {
			...
			$list = pdo_fetchall("SELECT * FROM " . tablename($this->tb_yuyue) . " WHERE weid = :weid and status = 1 and kfid = :openid ORDER BY reid DESC", array(':weid' => $weid, ':openid' => $openid), 'reid');
			$list_title = '切换主题';
		}
		$par = iunserializer($activity['par']);
		if (!empty($reid)) {
			...
			$params = array(':reid' => $reid, ':starttime' => strtotime($starttime), ':endtime' => strtotime($endtime));
			$status = $_GPC['status'];
			if ($status != '') {
				if ($status == 2) {
					$where .= " and ( status=2 or status=9 )";
	...
	public function doMobilePower()
	{
		...
		if ($_GPC['table'] == 'manage') {
			...
			if (pdo_update($this->tb_yuyue, $data, array('reid' => $reid, 'weid' => $_W['uniacid'])) === false) {
				$result['status'] = 0;
				$result['msg'] = '转移失败';
			} else {
				$result['status'] = 1;
		...
		if ($_GPC['table'] == 'case') {
			...
			if (pdo_update($this->tb_info, $data, array('reid' => $reid, 'rerid' => $rerid)) === false) {
				...
				$activity = pdo_get($this->tb_yuyue, array('weid' => $_W['uniacid'], 'reid' => $reid), array());
				$content = pdo_get($this->tb_info, array('reid' => $reid, 'rerid' => $rerid), array('member', 'mobile', 'address', 'xmid', 'price', 'num', 'restime', 'yuyuetime', 'createtime'));
				$add = $activity['is_addr'] == 1 ? "\\n地址：" . $content['address'] : "";
				if ($activity['is_time'] != 1) {
				...
				if ($_W['account']['level'] == ACCOUNT_SERVICE_VERIFY && !empty($activity['k_templateid'])) {
					$template = array("touser" => $_GPC['openid'], "template_id" => $activity['k_templateid'], "url" => $_W['siteroot'] . 'app/' . $this->createMobileUrl('manage_detail', array('reid' => $reid, 'id' => $rerid)), "topcolor" => "#FF0000", "data" => array('first' => array('value' => urlencode("请及时受理\\n"), 'color' => "#743A3A"), 'keyword1' => array('value' => urlencode(date('Y年m月d日 H:i', $content['createtime'])), 'color' => '#000000'), 'keyword2' => array('value' => urlencode("管理员派单"), 'color' => '#EF4F4F'), 'remark' => array('value' => urlencode("\\n姓名：" . $content['member'] . "\\n手机：" . $content['mobile'] . $add . "\\n" . $par['xmname'] . "：" . $xiangmu . $time), 'color' => "#01579b")));
					$this->send_template_message(urldecode(json_encode($template)));
				} else {
	...
	public function doMobilemanage_detail()
	{
		...
		if ($openid == $activity['kfid'] || $openid == $row['kf'] || $openid == $store['openid'] || $activity['guanli'] == '1' && $isstaff) {
			...
			$info_rd = pdo_fetchall('SELECT * FROM ' . tablename($this->tb_record) . ' WHERE rerid = :rerid ORDER BY id DESC', array(':rerid' => $row['rerid']));
			foreach ($info_rd as $key => $val) {
				$info_rd[$key]['manage'] = pdo_get($this->tb_staff, array('weid' => $weid, 'reid' => $reid, 'openid' => $val['openid']), array('nickname'));
				$info_rd[$key]['status'] = $this->get_status($reid, $val['status']);
				$info_rd[$key]['ostatus'] = $this->get_status($reid, $val['ostatus']);
				$info_rd[$key]['thumb'] = !empty($val['thumb']) ? iunserializer($val['thumb']) : '';
				$info_rd[$key]['createtime'] = date('Y-m-d H:i:s', $val['createtime']);
			...
			if ($_W['ispost']) {
				...
				$data = array('first' => array('value' => $par['mfirst'] . "\n", 'color' => "#743A3A"), 'keyword1' => array('value' => $row['member']), 'keyword2' => array('value' => $xiangmu['title'] . " - " . $xiangmu['price'] . "元"), 'keyword3' => array('value' => $times), 'keyword4' => array('value' => $status['name']), 'remark' => array('value' => $kfinfo . "\n" . $activity['mfoot'], 'color' => "#008000"));
				$check = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_record) . " WHERE rerid=:rerid and status=3", array('rerid' => $row['rerid']));
				$recorddata = array('rerid' => $row['rerid'], 'openid' => $openid, 'thumb' => $record['thumb'], 'info' => $record['kfinfo'], 'ostatus' => $row['status'], 'status' => $record['status'], 'createtime' => TIMESTAMP);
				...
				if ($par['sms'] != '0' && !empty($activity['smsid']) && $row['status'] == '0') {
					load()->func('communication');
					ihttp_post(murl('entry', array('do' => 'Notice', 'id' => $activity['smsid'], 'm' => 'dayu_sms'), true, true), array('mobile' => $row['mobile'], 'mname' => $row['member'], 'product' => $xiangmu['title'], 'status' => $status['name']));
				}
				$store_msg = '';
				if ($setting['store'] == 1 && pdo_tableexists('dayu_yuyuepay_plugin_store_store') && !empty($row['sid']) && $_GPC['status'] == '3') {
					$store = $this->get_store($row['sid']);
					$boss = $this->get_store_boss($store['bid']);
					$paylog = pdo_get('dayu_yuyuepay_plugin_store_paylog', array('weid' => $_W['uniacid'], 'sid' => $row['sid'], 'bid' => $store['bid'], 'reid' => $row['reid'], 'yid' => $row['rerid']), array());
					if (empty($paylog)) {
						$paylogdata = array('weid' => $_W['uniacid'], 'sid' => $row['sid'], 'bid' => $store['bid'], 'reid' => $row['reid'], 'yid' => $row['rerid'], 'price' => $row['price'], 'createtime' => TIMESTAMP);
						pdo_insert('dayu_yuyuepay_plugin_store_paylog', $paylogdata);
						pdo_update('dayu_yuyuepay_plugin_store_boss', array('money' => $boss['money'] + $paylogdata['price']), array('id' => $store['bid']));
						$store_msg = '店主余额增加' . $paylogdata['price'] . '元，';
					}
	...
	public function doMobileUpload()
	{
		...
		if ($_GPC['type'] == 2) {
			...
			$picurl = $upload_path . $images_path . $pic;
			$data = array('remit' => $images_path . $pic);
			pdo_update($this->tb_info, $data, array('rerid' => $_GPC['id']));
	...
	public function doMobileInfo()
	{
		...
		$titles = '更新资料';
		if ($operation == 'post') {
			$data = array('nickname' => $_GPC['nickname'], 'realname' => $_GPC['realname'], 'mobile' => $_GPC['mobile'], 'resideprovince' => $_GPC['province'], 'residecity' => $_GPC['city'], 'residedist' => $_GPC['area'], 'address' => $_GPC['address'], 'updatetime' => time());
			load()->model('mc');
		...
		if ($operation == 'sex') {
			$data = array('gender' => intval($_GPC['sex']));
			load()->model('mc');
			mc_update($uid, $data);
	...
	public function doMobilechangeAjax()
	{
		...
		if (!empty($id)) {
			$url = $_W['siteroot'] . 'app/' . $this->createMobileUrl('detail', array('reid' => $row['reid'], 'id' => $row['rerid']));
			$datas = array('first' => array('value' => $par['mfirst'] . "\n", 'color' => "#743A3A"), 'keyword1' => array('value' => $row['member']), 'keyword2' => array('value' => $xiangmu['title'] . " - " . $xiangmu['price'] . "元"), 'keyword3' => array('value' => $times), 'keyword4' => array('value' => $activity['state3']), 'remark' => array('value' => "\n" . $activity['mfoot'], 'color' => "#008000"));
			$acc = WeAccount::create($_W['acid']);
			$acc->sendTplNotice($row['openid'], $activity['m_templateid'], $datas, $url, "#FF0000");
			pdo_update($this->tb_info, $data, array('rerid' => $id));
	...
	public function get_store_boss($bid)
	{
		...
		if (pdo_tableexists('dayu_yuyuepay_plugin_store_store')) {
			return pdo_get('dayu_yuyuepay_plugin_store_boss', array('weid' => $_W['uniacid'], 'id' => $bid), array());
		}
	}
	public function get_store($sid)
	{
		global $_GPC, $_W;
		if (pdo_tableexists('dayu_yuyuepay_plugin_store_store')) {
			return pdo_get('dayu_yuyuepay_plugin_store_store', array('weid' => $_W['uniacid'], 'id' => $sid), array());
		}
	}
	...
	{
		global $_GPC, $_W;
		return pdo_get('users', array('uid' => $uid), array());
	}
	public function get_isrole($reid, $uid)
	{
		global $_GPC, $_W;
		return pdo_get($this->tb_role, array('weid' => $_W['uniacid'], 'reid' => $reid, 'roleid' => $uid), array());
	}
	public function get_staff($openid)
	{
		global $_GPC, $_W;
		return pdo_get($this->tb_staff, array('weid' => $_W['uniacid'], 'openid' => $openid), array());
	}
	public function get_manger($reid, $openid)
	{
		global $_GPC, $_W;
		return pdo_get($this->tb_yuyue, array('weid' => $_W['uniacid'], 'reid' => $reid, 'kfid' => $openid), array());
	}
	public function get_xiangmu($reid, $xmid)
	{
		global $_GPC, $_W;
		return pdo_get($this->tb_item, array('weid' => $_W['uniacid'], 'id' => $xmid, 'reid' => $reid, 'isshow' => 1), array());
	}
	public function get_info($uid)
	{
		global $_W;
		return pdo_get('mc_members', array('uniacid' => $_W['uniacid'], 'uid' => $uid), array());
	}
	public function get_yuyuepay($reid)
	{
		global $_W;
		return pdo_get($this->tb_yuyue, array('weid' => $_W['uniacid'], 'reid' => $reid), array());
	}
	...
	public function isHy($openid)
	{
		global $_W;
		load()->model('mc');
		$card = pdo_get('mc_card_members', array('uniacid' => $_W['uniacid'], 'openid' => $openid), array());
		if (empty($card)) {
			return false;
		} else {
			return true;
	...
	public function getFollow()
	{
		global $_GPC, $_W;
		require 'fans.mobile.php';
		$p = pdo_fetch("SELECT follow FROM " . tablename('mc_mapping_fans') . " WHERE uniacid = :weid AND openid = :openid LIMIT 1", array(":weid" => $weid, ":openid" => $openid));
		if ($p['follow'] == 0) {
	...
	private function checkAuth($weid, $openid)
	{
		...
		if (empty($_W['member']['uid']) && empty($settings['passport']['focusreg'])) {
			$fan = pdo_get('mc_mapping_fans', array('uniacid' => $weid, 'openid' => $openid));
			if (!empty($fan)) {
				$fanid = $fan['fanid'];
			} else {
				if (empty($openid)) {
				...
				$post = array('acid' => $_W['acid'], 'uniacid' => $weid, 'updatetime' => time(), 'openid' => $openid, 'follow' => 0);
				pdo_insert('mc_mapping_fans', $post);
				$fanid = pdo_insertid();
			}
			if (empty($fan['uid'])) {
				pdo_insert('mc_members', array('uniacid' => $weid));
				$uid = pdo_insertid();
				$_W['member']['uid'] = $uid;
				$_W['fans']['uid'] = $uid;
	...
	public function doMobileLocateStore()
	{
		...
		$store = pdo_fetch("SELECT name,address,mobile,lat,lng FROM " . tablename('dayu_yuyuepay_plugin_store_store') . " WHERE weid = :weid AND id = :id", array(':weid' => $weid, ':id' => $_GPC['id']));
...
function tpl_app_form_avatar_thumb($name, $value = '', $id, $mode)
{
	...
	';
	$href = url('entry', array('do' => 'Upthumb', 'id' => $id, 'm' => 'dayu_yuyuepay', 'mode' => $mode), true, true);
	$html .= "<script>
