...
function(a) {
    ...
    var b = function(b) {
        ...
        function j(a) {
            ...
            if (a.preventDefault(), !c.opened && (c.open(), c.params.scrollToInput && !g())) {
                ...
                f = b[0].offsetHeight - d - c.container.height(),
                h = b[0].scrollHeight - d - c.container.height(),
                j = c.input.offset().top - d + c.input[0].offsetHeight,
                j > f && (k = b.scrollTop() + j - f, k + f > h && (i = k + f - h + e, f === h && (i = c.container.height()), b.css({
                    "padding-bottom": i + "px"
                })), b.scrollTop(k, 300))
            }
        }
        ...
        f = a.device.ios || navigator.userAgent.toLowerCase().indexOf("safari") >= 0 && navigator.userAgent.toLowerCase().indexOf("chrome") < 0 && !a.device.android,
        ...
        c.initPickerCol = function(b, d) {
            ...
            function F(b) {
                if (!u && !t) {
                    ...
                    x = (new Date).getTime(),
                    s = !0,
                    z = B = a.getTranslate(h.wrapper[0], "y")
                }
            }
            function G(b) {
                var d, e;
            ...
            h.calcSize = function() {
                ...
                var b, d;
                b = 0,
                d = h.container[0].offsetHeight,
                k = h.wrapper[0].offsetHeight,
                l = h.items[0].offsetHeight,
                m = l * h.items.length,
                n = d / 2 - m + l / 2,
                o = d / 2 - l / 2,
                h.width && (b = h.width, parseInt(b, 10) === b && (b += "px"), h.container.css({
                    width: b
                })),
            ...
            h.calcSize(), h.wrapper.transform("translate3d(0," + o + "px,0)").transition(0), h.setValue = function(b, d, e) {
                ...
                f = h.wrapper.find('.picker-item[data-picker-value="' + b + '"]').index(),
                "undefined" != typeof f && -1 !== f && (g = -f * l + o, h.wrapper.transition(d), h.wrapper.transform("translate3d(0," + g + "px,0)"), c.params.updateValuesOnMomentum && h.activeIndex && h.activeIndex !== f && (a.cancelAnimationFrame(q), h.wrapper.transitionEnd(function() {
                    a.cancelAnimationFrame(q)
                }), r()), h.updateItems(f, g, d, e))
            },
            h.updateItems = function(b, d, e, g) {
                var i, j;
                "undefined" == typeof d && (d = a.getTranslate(h.wrapper[0], "y")),
                "undefined" == typeof b && (b = -Math.round((d - o) / l)),
                0 > b && (b = 0),
                b >= h.items.length && (b = h.items.length - 1),
                i = h.activeIndex,
                h.activeIndex = b,
                h.wrapper.find(".picker-selected").removeClass("picker-selected"),
                c.params.rotateEffect && h.items.transition(e),
                j = h.items.eq(b).addClass("picker-selected").transform(""),
                (g || "undefined" == typeof g) && (h.value = j.attr("data-picker-value"), h.displayValue = h.displayValues ? h.displayValues[b] : h.value, i !== b && (h.onChange && h.onChange(c, h.value, h.displayValue), c.updateValue())),
                c.params.rotateEffect && ((d - (Math.floor((d - o) / l) * l + o)) / l, h.items.each(function() {
                    var b = a(this),
                    c = b.index() * l,
                    e = o - d,
                    g = c - e,
                    ...
                    k > 180 && (k = 180),
                    -180 > k && (k = -180),
                    Math.abs(i) > j ? b.addClass("picker-item-far") : b.removeClass("picker-item-far"),
                    b.transform("translate3d(0, " + ( - d + o) + "px, " + (f ? -110 : 0) + "px) rotateX(" + k + "deg)")
                }))
            },
            d && h.updateItems(0, o, 0), s = !0, h.initEvents = function(b) {
                var c = b ? "off": "on";
                h.container[c](a.touchEvents.start, F),
                h.container[c](a.touchEvents.move, G),
                h.container[c](a.touchEvents.end, H),
                h.items[c]("click", I)
            },
        ...
        c.layout = function() {
            var d, e, f, a = "",
            b = "";
            for (c.cols = [], e = "", d = 0; d < c.params.cols.length; d++) f = c.params.cols[d],
            e += c.columnHTML(c.params.cols[d]),
            c.cols.push(f);
            b = "weui-picker-modal picker-columns " + (c.params.cssClass || "") + (c.params.rotateEffect ? " picker-3d": ""),
            a = '<div class="' + b + '">' + (c.params.toolbar ? c.params.toolbarTemplate.replace(/{{closeText}}/g, c.params.toolbarCloseText).replace(/{{title}}/g, c.params.title) : "") + '<div class="picker-modal-inner picker-items">' + e + '<div class="picker-center-highlight"></div>' + "</div>" + "</div>",
            c.pickerHTML = a
        },
        c.params.input && (c.input = a(c.params.input), c.input.length > 0 && (c.params.inputReadOnly && c.input.prop("readOnly", !0), c.inline || c.input.on("click", j), c.params.inputReadOnly && c.input.on("focus mousedown",
        function(a) {
            a.preventDefault()
        }))),
        c.inline || a("html").on("click", k),
        c.opened = !1,
        c.open = function() {
            var b = g();
            c.opened || (c.layout(), b ? (c.pickerHTML = '<div class="popover popover-picker-columns"><div class="popover-inner">' + c.pickerHTML + "</div></div>", c.popover = a.popover(c.pickerHTML, c.params.input, !0), c.container = a(c.popover).find(".weui-picker-modal"), a(c.popover).on("close",
            function() {
                l()
            })) : c.inline ? (c.container = a(c.pickerHTML), c.container.addClass("picker-modal-inline"), a(c.params.container).append(c.container)) : (c.container = a(a.openPicker(c.pickerHTML)), a(c.container).on("close",
            function() {
                l()
            })), c.container[0].f7Picker = c, c.container.find(".picker-items-col").each(function() {
                var a = !0; (!c.initialized && c.params.value || c.initialized && c.value) && (a = !1),
                c.initPickerCol(this, a)
        ...
        c.destroy = function() {
            c.close(),
            c.params.input && c.input.length > 0 && c.input.off("click focus", j),
            a("html").off("click", k),
    ...
    a.openPicker = function(b, c) {
        var d, e;
        return a.closePicker(),
        d = a("<div class='weui-picker-container " + (c || "") + "'></div>").appendTo(document.body),
        d.show(),
        d.addClass("weui-picker-container-visible"),
        e = a(b).appendTo(d),
        e.width(),
        e.addClass("weui-picker-modal-visible"),
        e
    },
    ...
    a.fn.picker = function(c) {
        ...
        return this.each(function() {
            var e, f, g, h;
            this && (e = a(this), f = e.data("picker"), f || (c = c || {},
            g = e.val(), void 0 === c.value && "" !== g && (c.value = c.cols.length > 1 ? g.split(" ") : [g]), h = a.extend({
                input: this
            },
        ...
        this.each(function() {
            ...
            },
            {
                ...
                onChange: function(a, b, d) {
                    ...
                    k = +new Date(g(b, d)),
                    l = !0,
                    c.min && (m = +new Date("function" == typeof c.min ? c.min() : c.min), m > k && (a.setValue(j), l = !1)),
                    c.max && (n = +new Date("function" == typeof c.max ? c.max() : c.max), k > n && (a.setValue(j), l = !1)),
                    l && (j = b)
                },
